...
ntrials = 50; % it has to be divisible by K
nvoxels = 50;
...
%     cvglmnetPlot(cvfit(i));
    ...
    voxel.used(:,i) = fit(i).beta ~= 0;
    voxel.remain(:,i) = fit(i).beta == 0;
    voxInd(i).used = find(voxel.used(:,i));
    voxInd(i).remain = find(voxel.used(:,i) == 0);
...
% for i = 1:5
%     hit.amount(i) = sum(voxel.used(1:numsignal,i));
%     hit.rate(i) = sum(voxel.used(1:numsignal,i)) / numsignal;
% end
% hit.amount
% hit.rate
% 
